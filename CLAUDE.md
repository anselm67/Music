...
**Current checkpoint status:** `stave-primary-grid` is the **best** model (D=128) — sys IoU **0.971** and it *flattens* the bottom-of-page drift: stave top/bottom error at bins 12–14 drops to **~2.4–3px** (vs ~5–6px for `stave-primary`, ~12–13px for every prior ltrb run), n-weighted mean top_err **~1.4px** (vs `stave-primary` 2.2px), clean `val/stave_l1_px` **1.04px**. Freezing the stave anchors to a uniform grid `(i+0.5)/M` and routing each GT stave to its nearest anchor (free / non-contiguous assignment) makes every query train at ~uniform frequency on a count-independent spatial band — so the rarely-fired high slots, the last source of drift, finally learn their position (the lever count-conditional anchoring missed). Prior best `stave-primary` (sys IoU 0.951; staves primary, systems derived as stave hull, no system→stave group attention) — both are D=128 half-size. NB: `val/stave_l1` is **not** a clean cross-architecture metric (logging-key collision in `StafferModule._step` — logged twice under the same key); use `val/stave_l1_px` (clean) and trust the eval table in `docs/staffer-training.html`. Prior: `enhanced2-ltrb-anchor` (sys IoU 0.914); `enhanced2-ltrb-anchor-iter` **NEGATIVE**; enhanced3 (cy_delta) reverted; enhanced2-staffer-small worse; enhanced2-vflip negative; enhanced2-sampler negative. Next: full-size retrain of `stave-primary-grid` (D=256/H=8/mlp=1024).
