...
**Current checkpoint status:** `stave-primary` is the **best** model (D=128) — sys IoU **0.951** and it finally bends the bottom-of-page drift: stave top/bottom error at bins 12–14 drops to ~5–6px (vs ~12–13px for every prior ltrb run), n-weighted mean top_err ~2.2px vs the anchor's 4.4px. Making staves the primary, directly-anchored stream (systems derived as their stave hull; no system→stave group attention) is what moved the bottom bins where count-conditional anchoring could not. NB: `val/stave_l1` is **not** a clean cross-architecture metric (logging-key collision in `StafferModule._step` — logged twice under the same key) and disagrees with the per-bin pixel eval; trust the eval table in `docs/staffer-training.html`. Prior: `enhanced2-ltrb-anchor` (sys IoU 0.914, prev best ltrb); `enhanced2-ltrb-anchor-iter` **NEGATIVE** (iterative refinement dropped); enhanced3 (cy_delta) reverted; enhanced2-staffer-small worse; enhanced2-vflip negative; enhanced2-sampler negative. Next: full-size retrain of stave-primary (D=256/H=8/mlp=1024); `stave-primary-grid` (frozen anchor grid + free non-contiguous assignment) is code-complete on this branch and pending.
